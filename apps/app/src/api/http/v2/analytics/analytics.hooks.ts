...
export const usePlatformAnalyticsV2Query = (
	...
	return useQuery<PlatformAnalyticsPayload>({
		queryKey: useOrganizationQueryKey(
			ANALYTICS_V2_QUERY_KEYS.root(params),
		),
...
export const useTenantAnalyticsV2Query = (
	...
	return useQuery<AnalyticsPayload>({
		queryKey: useOrganizationQueryKey(
			ANALYTICS_V2_QUERY_KEYS.tenant(tenantId ?? "", params),
		),
		queryFn: () => {
			if (!tenantId) {
...
export const useTenantStatementsV2Query = (
	...
	return useQuery<PaginatedStatementsListResult>({
		queryKey: useOrganizationQueryKey(
			ANALYTICS_V2_QUERY_KEYS.statements(params, tenantId),
		),
